function [chemin,NombreDeNoeud] = Astar(Nodes,NodeList,RootNode,TargetNode,Map_plan2node)
...
NodeList(RootNode).h = sqrt( square(NodeList(TargetNode).x - NodeList(RootNode).x )+square(NodeList(TargetNode).y-NodeList(RootNode).y)); %Heuristique de Rootnode
while length(Open) ~= 0
    nbest = Open(1);
    chemin(end+1)=nbest;
    Open = Open(2:end);
    ...
    for i = 1:length(NodeList(nbest).Neigh)                    %Pour chaque fils
        if(not(ismember(NodeList(nbest).Neigh(i),Closed)))     %Pas dans Closed
            if(not(ismember(NodeList(nbest).Neigh(i),Open)))   %Pas dans Open
                ...
                NodeList(NodeList(nbest).Neigh(i)).h = sqrt(square(NodeList(TargetNode).x-NodeList(NodeList(nbest).Neigh(i)).x)+square(NodeList(TargetNode).y-NodeList(NodeList(nbest).Neigh(i)).y));  %Heuristique du nBest's child
            else
...
for i = length(chemin):-1:2
    if(NodeList(chemin(i)).parent == chemin(i-1))     %Trouver le parent
